...
import { BOX_BLUR_FRAG_SHADER, DEFAULT_FRAG_SHADER, DEFAULT_VERT_SHADER } from '../shader';
import { blitFrameBuffer, blitTexture, createFrameBuffer, createShader } from '../utils';

export class BoxBlurFilter implements IFilter {
    private static vert = DEFAULT_VERT_SHADER;
    private static frag = BOX_BLUR_FRAG_SHADER;
    ...
    public filter(
        ...
        if (!this.program) {
            const program = createShader(gl, BoxBlurFilter.vert, BoxBlurFilter.frag);
            if (!program) {
                console.error('[BoxBlurFilter] Unable to create program');
                return;
            }
        ...
        if (!this.blitProgram) {
            ...
            if (!program) {
                console.error('[BoxBlurFilter] Unable to create blit program');
                return;
            }
